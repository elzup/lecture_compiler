...
class Calcurator {
	...
	public Calcurator() {
		symbolMap = new HashMap<String, Double>();
	...
	private Double calculate(List<Token> reversePolishList) {
		...
		for (Token token : reversePolishList) {
			...
			if (token instanceof UnarioOperator) {
				double operand1 = stack.pop();
				System.out.println(" 演算: " + operand1);
				stack.push(((UnarioOperator) token).calc(operand1));
			}
				...
				// 文字列から数値に変換して push
				stack.push((double) ((IntConstant) token).intValue());
//			}
...
//				stack.push(((DoubleConstant) token).doubleValue());
				...
				double val = symbolMap.get(token.toString());
